// Dev-only API endpoint for AI-powered blackout poetry.
// Loaded automatically by Create React App's webpack-dev-server.
// Requires ANTHROPIC_API_KEY in .env (or .env.local). Restart `npm start`
// after editing this file or .env.
  ...
  app.post('/api/blackout', async (req, res) => {
    if (!process.env.ANTHROPIC_API_KEY) {
      return res.status(500).json({
        error:
          'ANTHROPIC_API_KEY is not set. Add it to .env (or .env.local) and restart `npm start`.',
      });
    }

    const { prompt, words, count } = req.body || {};
    ...
    try {
      ...
      // Normalize lines: integers, ascending, in-range, no duplicates across lines.
      const seen = new Set();
